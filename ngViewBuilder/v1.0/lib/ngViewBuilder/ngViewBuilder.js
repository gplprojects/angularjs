...
.service('$ngViewLoader', ['$rootScope', '$q', '$http', '$log', function ($rootScope, $q, $http, $log) {
     ...
     */
    this.loadMeta = function (viewName, callback) {
        $http({
            url: 'meta/' + viewName + '-meta.js',
            method: "GET",
    ...
    function buildPanel(scope, control, key, parentEl, dataPath, isCallback, model, isCallback) {
        ...
        switch(control.type) {
            ...
            case "chartpanel":
                ...
                scope.$schema.config[control.name] = defaultConfig; //Overwrite default with user config
