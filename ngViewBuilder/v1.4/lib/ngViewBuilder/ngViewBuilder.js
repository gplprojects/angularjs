...
.service('$ngViewLoader', ['$rootScope', '$q', '$http', '$log', function ($rootScope, $q, $http, $log) {
    ...
    this.loadMeta = function (viewName, callback, metaPath, url, sync) {
        ...
        if (sync === true) {
            ...
            .then(function(res) { callback(res.data); }, function () {
        ...
        switch (options.type) {
            case "post":
                    $http.post(options.action,
                        ...
                        .then(function (res) { $scope.$interceptResponse(res.data, options, false); }, function (res) { $scope.$interceptResponse(res, options, true); });
                break;
